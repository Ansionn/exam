...
$(function() {
	...
	var message = ["영문, 숫자만 가능. 6~12자로 입력해 주세요.",
		...
		"비밀번호와 비밀번화 확인란은 값이 일치해야 합니다.",
		"-포합 입력해 주세요. 예시) 010-0000-0000"];
	...
	$("#idConfirmBtn").click(function() {
		if (!formCheck($('#userId'), $('.error:eq(0)'), "아이디를")) return;
			...
			$.ajax({
				...
				error: function() {
					alert('사이트 접속에 문제로 정상 작동하지 못하였습니다. 잠시후 다시 시도해 주세요.');
				},
				success: function(resultData) {
					console.log("resutData : " + resultData);
					if (resultData == "1") {
						$("#userId").parents(".form-group").find(".error").html("현재 사용중인 아이디 입니다.");
					} else if (resultData == "2") {
						$("#userId").parents(".form-group").find(".error").html("사용 가능한 아이디 입니다.");
						idConfirm = 2;
					}
				}
			});
	...
	$("#joinInsert").click(function() {
		...
		else if (idConfirm != 2) { alert("아이디 중복 체크를 진행해 주세요."); }
		else {
			$("#email").val($("#emailName").val() + "@" + $("#emailDomail").val());
